...
namespace game
{
    void Pong::LoadEnd(ecs::Scene &scene)
    {
        ...
        this->world.assign(Winner, ecs::Position{800, 50});
        this->world.assign(Winner, ecs::Text({this->winner}));
        this->world.assign(Winner, ecs::FontSize({25}));
        ...
        this->world.assign(ButtonBack, ecs::Clickable{false, [this](ecs::Clickable&) {
            std::cout << "BackButton pressed" << std::endl;
            // LoadGame(this->world.getSceneById(3));
            // this->world.switchToScene(3);
        }});
    }
}